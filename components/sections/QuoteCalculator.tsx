...
export default function QuoteCalculator() {
  ...
  const formatMoney = (usdAmount: number) => {
    ...
    if (currency === 'EUR') {
      const eur = usdAmount * eurRate;
      return `€${eur.toLocaleString('en-EU', { maximumFractionDigits: 0 })}`;
    }
  ...
  const handleRevealPrice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedSaved = [newSavedItem, ...savedQuotes];
              ...
              <div className="inline-flex p-1 bg-ean-black-pure/80 border border-ean-gold/20 rounded-xl">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                    appMode === 'calculator' ? 'bg-ean-gold text-ean-burgundy-night font-bold shadow-md' : 'text-ean-muted-light hover:text-white'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                    appMode === 'fleet' ? 'bg-ean-gold text-ean-burgundy-night font-bold shadow-md' : 'text-ean-muted-light hover:text-white'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                    appMode === 'tariff' ? 'bg-ean-gold text-ean-burgundy-night font-bold shadow-md' : 'text-ean-muted-light hover:text-white'
                  }`}
                >
                ...
                {(['USD', 'NGN', 'EUR'] as const).map((curr) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                      currency === curr ? 'bg-ean-gold/20 text-ean-gold border border-ean-gold/40' : 'text-ean-muted-light hover:text-white'
                    }`}
                  >
        ...
        {appMode === 'calculator' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 bg-linear-to-b from-ean-black-accent/90 to-ean-burgundy-deep/60 border border-ean-gold/20 rounded-2xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl space-y-8">
                      ...
                      {isLoadingApi ? (
                        ...
                        filteredAircraft.map((ac) => (
                          <button
                            ...
                            className={`w-full text-left p-3 rounded-lg flex items-center justify-between hover:bg-ean-gold/10 transition-colors cursor-pointer ${
                              selectedAircraft.id === ac.id ? 'bg-ean-gold/15 border border-ean-gold/40' : ''
                            }`}
                          >
                            <div>
                              <div className="flex items-center gap-2">
              ...
              <div className="space-y-3 pt-4 border-t border-ean-gold/15">
                <span className="block text-xs font-semibold tracking-wider text-ean-gold uppercase flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-ean-gold" /> Executive Ground Support Add-ons
                </span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  
                  <label className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
                    addOns.vipLounge ? 'bg-linear-to-r from-ean-gold/15 to-ean-burgundy-deep/30 border-ean-gold text-white shadow-[0_0_12px_rgba(196,149,42,0.15)]' : 'bg-ean-black-pure/60 border-white/10 text-ean-muted-light hover:border-ean-gold/30'
                  }`}>
                  ...
                  <label className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
                    addOns.catering ? 'bg-linear-to-r from-ean-gold/15 to-ean-burgundy-deep/30 border-ean-gold text-white shadow-[0_0_12px_rgba(196,149,42,0.15)]' : 'bg-ean-black-pure/60 border-white/10 text-ean-muted-light hover:border-ean-gold/30'
                  }`}>
                    <div className="flex items-center gap-2.5">
                      <input
                        type="checkbox"
                  ...
                  <label className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
                    addOns.gpuPower ? 'bg-linear-to-r from-ean-gold/15 to-ean-burgundy-deep/30 border-ean-gold text-white shadow-[0_0_12px_rgba(196,149,42,0.15)]' : 'bg-ean-black-pure/60 border-white/10 text-ean-muted-light hover:border-ean-gold/30'
                  }`}>
                    <div className="flex items-center gap-2.5">
                      <input
                        type="checkbox"
                  ...
                  <label className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${
                    addOns.waterService ? 'bg-linear-to-r from-ean-gold/15 to-ean-burgundy-deep/30 border-ean-gold text-white shadow-[0_0_12px_rgba(196,149,42,0.15)]' : 'bg-ean-black-pure/60 border-white/10 text-ean-muted-light hover:border-ean-gold/30'
                  }`}>
                    <div className="flex items-center gap-2.5">
                      <input
                        type="checkbox"
          ...
          <div className="bg-linear-to-b from-ean-black-accent/90 to-ean-burgundy-deep/60 border border-ean-gold/20 rounded-2xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {AIRCRAFT_DATASET.map((ac) => (
                <div
                  key={ac.id}
                  className={`p-5 rounded-2xl border transition-all duration-300 space-y-4 flex flex-col justify-between ${
