...
export function calculateEstimatedQuote(
  input: QuoteCalculationInput,
): QuoteCalculationResult {
  const aircraft = input.customAircraft ?? getAircraftById(input.aircraftId);
  if (!aircraft) {
    throw new Error(`Unknown aircraftId: ${input.aircraftId}`);
  }
  ...
  const paxCount = Math.max(
    1,
    Math.min(input.passengers, aircraft.maxPassengers),
  );
  ...
  const landingFee = isIntl
    ? Math.round(aircraft.mtowKg * 0.022)
    : Math.round(aircraft.mtowKg * 0.012);
  ...
  return {
    ...
    locationName:
      input.location === "lagos"
        ? "Murtala Muhammed Int'l Airport, Lagos (MMIA)"
        : "Nnamdi Azikiwe Int'l Airport, Abuja (DNAA)",
    operationName:
      input.operation === "domestic"
        ? "Domestic Flight"
        : "International Flight",
    movementName:
      input.movement === "weekday" ? "Weekday Schedule" : "Weekend Schedule",
    stayName: isOvernight
      ? `${nights} Night${nights > 1 ? "s" : ""} Overnight`
      : "Same-day Turnaround",
    passengers: paxCount,
    breakdown,
    totalUsd,
