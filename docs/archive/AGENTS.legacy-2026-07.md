# ⚠️ ARCHIVED — NOT ACTIVE INSTRUCTIONS

> **This document is inactive historical material, retained for reference only.**
> It describes plans that were never built (the Sanity CMS, an earlier
> `lib/seo.ts` architecture, the original admin spec) alongside conventions the
> codebase has since moved off.
>
> **The active instructions are the root `AGENTS.md`.** Read that instead.
> Nothing below describes this codebase as it stands, and nothing below should
> be followed, cited as a convention, or used to justify a change. Everything
> from the horizontal rule down is preserved verbatim as a record of what was
...
*Historical reference. Superseded by the root `AGENTS.md`.*
